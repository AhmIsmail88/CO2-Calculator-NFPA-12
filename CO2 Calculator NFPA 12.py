...
def export_pdf(path: str, system_data: dict, results: dict) -> None:
    ...
    title = "<font color='#1F4E79'>CO₂ Total Flooding Calculator Report (Conceptual) – NFPA 12</font>"
    ...
    summary = [
        ["Key Summary", "Value"],
        ["Total CO₂ Required", f"{results['total_lb']:.2f} lb   ({results['total_kg']:.2f} kg)"],
...
class CO2App(ctk.CTk):
    ...
    def _build_inputs(self):
        ...
        self.unit_label_var = ctk.StringVar(value=UNIT_OPTIONS[0][1])
        ctk.CTkLabel(f, text="Input Units (scrollable)").grid(row=3, column=0, sticky="w", padx=12, pady=(6, 2))
        self.unit_frame = ctk.CTkScrollableFrame(f, height=70)
        self.unit_frame.grid(row=3, column=1, sticky="ew", padx=12, pady=(6, 2))
        for idx, (label, unit, _) in enumerate(UNIT_OPTIONS):
            ctk.CTkRadioButton(
                self.unit_frame,
                text=label,
                value=unit,
                variable=self.unit_label_var,
                command=self._update_dimension_labels,
        ...
        ctk.CTkLabel(f, text="Cylinder Banks").grid(row=9, column=0, sticky="w", padx=12, pady=(12, 4))
        ...
        self.opt_reserve.grid(row=10, column=0, columnspan=2, sticky="w", padx=12, pady=(0, 6))

        # NEW: Nozzle tag (for schematic)
        _, self.nozzle_tag = self._add_entry(f, "Nozzle Tag (schematic)", 11, "E1-N1")

        # Buttons
        self.calc_btn = ctk.CTkButton(f, text="Calculate", command=self.on_calculate)
        self.calc_btn.grid(row=12, column=0, padx=12, pady=(18, 10), sticky="ew")

        self.pdf_btn = ctk.CTkButton(f, text="Export PDF", command=self.on_export_pdf)
        self.pdf_btn.grid(row=12, column=1, padx=12, pady=(18, 10), sticky="ew")

        note = "v1: Cylinder size fixed at 100 lb (~45.4 kg)."
        ctk.CTkLabel(f, text=note).grid(row=13, column=0, columnspan=2, sticky="w", padx=12, pady=(0, 10))
